...
export default class EmployeeService {
    ...
    static post = (
        ...
        return authApi.post<IEmployee>(`/dep_${depId}/students/data`, employee, config);
    };
    static put = ({ depId, employee }: UpdateParams, config?: AxiosRequestConfig): Promise<AxiosResponse<void>> => {
        return authApi.put<void>(`/dep_${depId}/students/data`, employee, config);
    };
    static delete = ({ depId, studentId }: DeleteParams, config?: AxiosRequestConfig): Promise<AxiosResponse<void>> => {
        return authApi.delete<void>(`/dep_${depId}/students/${studentId}`, config);
    };
}
